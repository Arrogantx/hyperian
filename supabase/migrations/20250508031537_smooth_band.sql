/*
  ...
  1. Changes
    - Add public INSERT policy with proper constraints
    - Fix case-sensitive address comparison in admin policy
    - Add index for case-insensitive address lookups
    - Maintain existing policies for SELECT and admin management

  2. Security
    - Enable RLS on whitelist table
    - Allow public SELECT for status checking
    - Allow public INSERT with constraints
    - Restrict admin actions to specific wallet addresses
*/
...
-- Allow public to insert new entries with constraints
CREATE POLICY "Anyone can add to whitelist"
  ON whitelist
  FOR INSERT
  TO public
  WITH CHECK (
    -- Ensure status is 'standard' for public inserts
    status = 'standard' AND
    -- Ensure address is not empty
    length(address) > 0 AND
    -- Ensure added_by is not empty
    length(added_by) > 0
  );
